...
def show_overview(db):
    ...
    if not trades_df.empty:
        ...
        st.dataframe(
            trades_df[display_cols].head(20), use_container_width=True, height=400
        )
...
def show_agent_control():
    ...
    st.markdown(
        "Manage AI trading agents. Start, stop, or restart individual agents to find the best performing strategies."
    )
    ...
    with col1:
        if race_running:
            st.markdown(
                '<span class="badge-running">⚡ RUNNING</span>', unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<span class="badge-stopped">⏹ STOPPED</span>', unsafe_allow_html=True
            )
        ...
        if all_trades:
            ...
            if "strategy_name" in trades_df.columns:
                agent_trades = trades_df[
                    trades_df["strategy_name"].str.contains(
                        agent_name, case=False, na=False
                    )
                ]
    ...
    for idx, agent in enumerate(
        sorted(agents, key=lambda a: a.get("total_pnl", 0), reverse=True)
    ):
        col = cols[idx % 4]
        with col:
...
def show_api_keys():
    ...
    st.markdown(
        "Manage your exchange and service API keys. Keys are stored securely in environment variables."
    )
    ...
    # Exchange selector
    current_exchange = os.environ.get(
        "EXCHANGE_NAME", keys.get("EXCHANGE_NAME", "coindcx")
    )
    exchange_selection = st.selectbox(
        "Active Exchange",
        ["CoinDCX", "Binance"],
        index=0 if current_exchange == "coindcx" else 1,
        key="exchange_selection",
    )

    if st.button(
        "💾 Set Active Exchange", use_container_width=True, key="set_exchange"
    ):
        selected = "coindcx" if exchange_selection == "CoinDCX" else "binance"
        keys["EXCHANGE_NAME"] = selected
        os.environ["EXCHANGE_NAME"] = selected
        with open(api_key_file, "w") as f:
            json.dump(keys, f, indent=2)
        st.success(f"Switched to {exchange_selection}")
    ...
    with col1:
        ...
        cd_key = st.text_input(
            "CoinDCX API Key",
            value=keys.get("COINDCX_API_KEY", ""),
            type="password",
            key="cd_key",
        )
        cd_secret = st.text_input(
            "CoinDCX API Secret",
            value=keys.get("COINDCX_API_SECRET", ""),
            type="password",
            key="cd_secret",
        )
    ...
    with col2:
        ...
        bn_key = st.text_input(
            "Binance API Key",
            value=keys.get("BINANCE_API_KEY", ""),
            type="password",
            key="bn_key",
        )
        bn_secret = st.text_input(
            "Binance Secret Key",
            value=keys.get("BINANCE_SECRET_KEY", ""),
            type="password",
            key="bn_secret",
        )

        if st.button(
            "💾 Save Binance Keys", use_container_width=True, key="save_binance"
        ):
            keys["BINANCE_API_KEY"] = bn_key
            keys["BINANCE_SECRET_KEY"] = bn_secret
            os.environ["BINANCE_API_KEY"] = bn_key
            os.environ["BINANCE_SECRET_KEY"] = bn_secret
            try:
                with open(api_key_file, "w") as f:
                    json.dump(keys, f, indent=2)
                st.success("Binance keys saved successfully!")
            except Exception as e:
                st.error(f"Failed to save: {e}")
    ...
    t_token = st.text_input(
        "Telegram Bot Token",
        value=keys.get("TELEGRAM_BOT_TOKEN", ""),
        type="password",
        key="tg_token",
    )
    t_chat = st.text_input(
        "Telegram Chat ID", value=keys.get("TELEGRAM_CHAT_ID", ""), key="tg_chat"
    )
    ...
    m_userid = st.text_input(
        "m.Stock User ID", value=keys.get("MSTOCK_USER_ID", ""), key="ms_user"
    )
    m_pass = st.text_input(
        "m.Stock Password",
        value=keys.get("MSTOCK_PASSWORD", ""),
        type="password",
        key="ms_pass",
    )
    m_pin = st.text_input(
        "m.Stock PIN", value=keys.get("MSTOCK_PIN", ""), type="password", key="ms_pin"
    )
...
def show_system_logs():
    ...
    if logs:
        ...
        st.code(
            log_text if log_text else "No matching log entries found", language="python"
        )
    else:
        st.info(
            "No log file found yet. Log file will be created when you start the trading system."
        )


# ============================================================================
# PAGE: PERSONAL TRADER MODE
# ============================================================================
def show_personal_trader():
    st.title("🤖 Personal Trader — India Crypto Alpha")
    st.markdown(
        "Autonomous paper trading with LLM-driven strategy evolution. Stay disciplined, ride the waterfall."
    )

    # Load state file and logs
    state_file = DATA_DIR / "personal_portfolio.json"
    log_file = Path(LOGS_DIR) / f"personal_trader_{datetime.now().date()}.jsonl"

    if not state_file.exists():
        st.warning(
            "No personal trader state found. Please start the trader with `./run_personal_trader.sh`."
        )
        st.code("python personal_trader.py  # or use the start script", language="bash")
        return
    ...
    try:
        ...
        st.error(f"Failed to read state file: {e}")
        return

    # Compute metrics
    initial_balance = state.get("initial_balance_inr", 100000)
    positions = state.get("positions", {})
    trade_history = state.get("trade_history", [])

    # Estimate current total value (using entry prices + cash)
    current_total = state.get("current_balance_inr", initial_balance)
    for pair, pos in positions.items():
        current_total += pos["quantity"] * pos["entry_price"]

    profit_pct = (
        ((current_total - initial_balance) / initial_balance) * 100
        if initial_balance > 0
        else 0
    )
    target_pct = 20.0  # VICTORY_MODE threshold

    # Waterfall status
    waterfall_level = None
    locked_pct = 0
    if profit_pct >= 20:
        waterfall_level = "VICTORY_MODE (20%+)"
        locked_pct = 100
    elif profit_pct >= 15:
        waterfall_level = "mostly_protected (75% locked)"
        locked_pct = 75
    elif profit_pct >= 10:
        waterfall_level = "half_protected (50% locked)"
        locked_pct = 50
    elif profit_pct >= 5:
        waterfall_level = "partial_protection (25% locked)"
        locked_pct = 25

    # Read latest log entry for regime and signal
    latest_log = None
    if log_file.exists():
        try:
            with open(log_file, "r") as f:
    ...
    # Display header with mode toggle (Read-only for now)
    col1, col2 = st.columns([3, 1])
    with col1:
        st.markdown(
            """
        <div style="background:linear-gradient(90deg, #2cc5a0, #ffd700); padding:16px; border-radius:12px; color:#000;">
            <h2 style="margin:0;">🏆 VICTORY MODE ACTIVE</h2> 
            <p style="margin:0;">Ultra conservative trading — 20% profit reached, capital locked</p>
        </div>
        """
            if waterfall_level and "VICTORY" in waterfall_level
            else f"""
        <div style="background:#1a1a2e; border:1px solid #262730; padding:16px; border-radius:12px;">
            <h2 style="margin:0;">🤖 PERSONAL TRADER — INDIA CRYPTO ALPHA</h2>
            <p style="margin:0;color:#838D9B;">Autonomous LLM-powered paper trading with strategy evolution</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
    with col2:
        st.session_state.personal_trader_mode = True

    st.markdown("---")

    # Top row: Profit progress + Waterfall
    col1, col2 = st.columns(2)

    with col1:
        st.markdown("### 📈 Profit Progress")
        # Progress bar
        progress = min(profit_pct / target_pct, 1.0) * 100
        st.progress(
            progress / 100.0, text=f"{profit_pct:+.2f}% (Target: +{target_pct:.0f}%)"
        )

        # Stats
        mcol1, mcol2 = st.columns(2)
        with mcol1:
            st.metric("Initial", f"₹{initial_balance:,.0f}")
        with mcol2:
            st.metric("Current", f"₹{current_total:,.0f}", f"{profit_pct:+.2f}%")

    with col2:
        st.markdown("### 🛡️ Waterfall Status")
        if locked_pct > 0:
            st.markdown(
                f"""
            <div style="padding:12px;background:#1a1a2e;border:1px solid #30363d;border-radius:8px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                    <span>🔒 <strong>{waterfall_level}</strong></span>
                    <span><strong>{locked_pct}% LOCKED</strong></span>
                </div>
                <div style="height:24px;background:#30363d;border-radius:4px;overflow:hidden;position:relative;">
                    <div style="width:{locked_pct}%;height:100%;background:linear-gradient(90deg, #2cc5a0, #00f5ff);"></div>
                    <div style="position:absolute;top:0;left:{100 - locked_pct}%;width:{(100 - locked_pct)}%;height:100%;background:#0a0a0f;"></div>
                </div>
                <div style="margin-top:8px;color:#8b949e;">
                    Available capital: <strong>₹{current_total * (1 - locked_pct / 100):,.0f}</strong>
                </div>
            </div>
            """,
                unsafe_allow_html=True,
            ...
            st.info(
                "No waterfall lock active — 0% locked, 100% of capital available for trading"
            )

    st.markdown("---")

    # Second row: Regime, Strategy, Evolution
    col1, col2, col3 = st.columns(3)

    with col1:
        st.markdown("### 🌐 Market Regime")
        regime = latest_log.get("regime", "unknown") if latest_log else "unknown"
        regimes_map = {
            "trending_up": ("Trending ↑", "#2cc5a0"),
            "trending_down": ("Trending ↓", "#ff6666"),
            "high_vol": ("High Vol 📊", "#ffaa32"),
            "mean_reverting": ("Mean Rev ↔️", "#00bfff"),
            "choppy": ("Choppy 🌫️", "#8b949e"),
        }
        label, color = regimes_map.get(regime, ("Unknown", "#8b949e"))
        st.markdown(
            f"""
        <div style="text-align:center;padding:12px;background:#1a1a2e;border:1px solid #30363d;border-radius:8px;">
            <div style="font-size:2rem;color:{color};">{label}</div>
            <div style="color:#8b949e;font-size:0.9rem;">{regime}</div>
        </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        strategy = state.get("strategy_params", {})
        st.markdown(
            f"""
        <ul style="list-style:none;padding:0;">
            <li>Risk per trade: <strong>{strategy.get("risk_per_trade", 0.02) * 100:.1f}%</strong></li>
            <li>Stop loss: <strong>{strategy.get("stop_loss_pct", 0.03) * 100:.1f}%</strong></li>
            <li>Signal threshold: <strong>{strategy.get("entry_signal_threshold", 0.7):.2f}</strong></li>
        </ul>
        """,
            unsafe_allow_html=True,
        )
        if latest_log:
            last_evol = latest_log.get("loop", "N/A")
            st.caption(f"Last evolution at loop {last_evol}")
    ...
    with col3:
        ...
        if recent_trades:
            ...
            wins = df[df.get("pnl", 0) > 0].shape[0]
            total = len(df)
            win_rate = (wins / total) * 100 if total > 0 else 0
            st.metric("Win Rate (Last 10)", f"{win_rate:.0f}%")
            st.metric("Total P&L", f"₹{df.get('pnl', pd.Series([0])).sum():,.0f}")
        else:
            st.info("No trades yet")

    st.markdown("---")

    # Third row: Research notes, evolution history, recent trades
    col1, col2 = st.columns([1, 1])

    with col1:
        st.markdown("### 📝 Latest Research Notes")
        # Fallback: show the last few evolution log entries
        evolution_logs = state.get("evolution_log", [])
        if evolution_logs:
            latest_evo = evolution_logs[-1]
            suggestion = latest_evo.get("suggestion", {})
            if suggestion:
                st.json(suggestion)
            else:
                st.text(str(latest_evo))
        else:
            st.info(
                "No evolution data yet. The LLM will generate research notes after the first few trades."
            )

    with col2:
        st.markdown("### 📜 Recent Paper Trades")
        if trade_history:
            recent_df = pd.DataFrame(trade_history[-20:])
            display_cols = ["timestamp", "pair", "side", "quantity", "price", "reason"]
...
def show_configuration():
    ...
    st.markdown(
        "Modify trading parameters and system settings. Changes will be saved to the .env file."
    )
    ...
    with col1:
        initial_portfolio = st.number_input(
            "Initial Portfolio (₹)",
            value=float(current.get("INITIAL_PORTFOLIO", "100000")),
            step=1000.0,
            format="%.0f",
        )
        risk_per_trade = (
            st.number_input(
                "Risk Per Trade (%)",
                value=float(current.get("RISK_PER_TRADE", "0.02")) * 100,
                step=0.5,
                format="%.1f",
            )
            / 100
        ...
        max_exposure = (
            st.number_input(
                ...
                value=float(current.get("MAX_PORTFOLIO_EXPOSURE", "0.10")) * 100,
                step=1.0,
                format="%.1f",
            )
            / 100
        ...
        stop_loss = (
            st.number_input(
                "Stop Loss (%)",
                value=float(current.get("STOP_LOSS_PERCENT", "0.03")) * 100,
                step=0.5,
                format="%.1f",
            )
            / 100
        ...
        daily_loss = (
            st.number_input(
                "Daily Max Loss (%)",
                value=float(current.get("DAILY_MAX_LOSS_PERCENT", "0.05")) * 100,
                step=0.5,
                format="%.1f",
            )
            / 100
        ...
        exchange_name = st.selectbox(
            ...
            ["binance", "coindcx"],
            index=0
            if current.get("EXCHANGE_NAME", "coindcx").lower() == "coindcx"
            else 1,
            help="Select the exchange for market data and trading",
        )
        paper_mode = st.selectbox(
            "Trading Mode",
            ["true", "false"],
            index=0 if current.get("PAPER_TRADING_MODE", "true") == "true" else 1,
        )
    ...
    with col2:
        ...
        num_agents = st.slider(
            ...
            min_value=1,
            max_value=50,
            value=int(current.get("NUM_RACE_AGENTS", "12")),
        )
        race_duration = st.slider(
            "Race Duration (hours)",
            min_value=1,
            max_value=168,
            value=int(current.get("RACE_DURATION_HOURS", "24")),
        )
        update_interval = st.number_input(
            "Update Interval (seconds)",
            value=int(current.get("RACE_UPDATE_INTERVAL_SEC", "10")),
            step=5,
            min_value=1,
        )
        evolution_interval = st.number_input(
            "Evolution Interval (minutes)",
            value=int(current.get("EVOLUTION_INTERVAL_MIN", "60")),
            step=15,
            min_value=1,
        )
        log_level = st.selectbox(
            ...
            index=1 if current.get("LOG_LEVEL", "INFO") == "INFO" else 0,
        )
    ...
    if st.button("💾 Save Configuration", type="primary", use_container_width=True):
        ...
        st.success(
            "Configuration saved! Restart the trading system for changes to take effect."
        )
...
def show_live_trades(db):
    ...
    with col1:
        open_trades = (
            df[df.get("status", pd.Series(["open"] * len(df))) == "open"]
    ...
    with col2:
        unrealized = (
            open_trades.get(
                "unrealized_pnl_inr", pd.Series([0] * len(open_trades))
            ).sum()
            if "unrealized_pnl_inr" in open_trades.columns
            else 0
        )
...
def show_risk_dashboard(db):
    ...
    if metrics["max_drawdown"] < -10:
        alerts.append(
            (
                "🔴 Critical Drawdown",
                f"Drawdown at {metrics['max_drawdown']:.2f}%",
                "error",
            )
        )
    ...
    if metrics["win_rate"] < 40 and metrics["total_trades"] > 5:
        alerts.append(
            ("🟡 Low Win Rate", f"Win rate at {metrics['win_rate']:.1f}%", "warning")
        )
    if metrics["profit_factor"] < 1 and metrics["total_trades"] > 5:
        alerts.append(
            (
                "🔴 Negative Profit Factor",
                f"PF at {metrics['profit_factor']:.2f}",
                "error",
            )
        )
...
def main():
    ...
    if page == "📊 Overview":
        ...
        show_agent_control()
    elif page == "🤖 Personal Trader":
        show_personal_trader()
